...
##### Acting on Rotation Events

In addition to re-loading the certificate data, the OpenUnison operator can be configured to call a webhook that can act on the update.  This can be used to, for instance push out an update of issuer documents.  To be able to receive this event, an `Application` and URL must be created.  See [how to build APIs using JavaScript](/javascript/#application-filters) for how to create this API.  Here is [an example API for AWS S3 and CloudFront](https://github.com/OpenUnison/openunison-sts-aws-update/blob/main/templates/applications/call-sts-update.yaml).

The API should be configured to use the `oauth2k8s-sts` `AuthenticationChain` and accept the following JSON payload:

```json
{
  "version": 2,
  "key_name": "name_of_key_in_openunison"
}
```

The `version` refers to the version of the `Secret` from the `SecretVersion` generated by the operator.  This way your webhook can wait for the latest version to get picked up before acting.
